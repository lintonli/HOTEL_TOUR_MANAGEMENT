...
export class HotelsService {
  ...
  getHotelbyTour(TourId: string): Observable<IHotel> {
    return this.http.get<IHotel>(this.Base_Url+TourId);
  }
  getHotelsbyTour(TourId: string): Observable<IHotel[]> {
  ...
  updateHotel(
    TourId: string,
    updatedHotel: HotelRequest
  ): Observable<HotelResponse> {
    return this.http.patch<HotelResponse>(this.Base_Url + TourId, updatedHotel);
  }
}
